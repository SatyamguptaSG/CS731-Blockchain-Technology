...
func (s *SmartContract) AcceptContract(ctx contractapi.TransactionContextInterface, contractID string, UserID string, UserType int) error {
	if UserType == 1 {
		return fmt.Errorf("employer acceptance option is presumed")
	}
...
// func (s *SmartContract) MakePayment(stub shim.ChaincodeStubInterface, contract Contract) pb.Response {
// 	// Prepare arguments for the chaincode invocation
// 	args := [][]byte{
// 		[]byte("makePayment"),
// 		[]byte(contract.EmployerId),
// 		[]byte(strconv.Itoa(contract.Payment)),
// 	}

// 	// Invoke the chaincode on channel two
// 	response := stub.InvokeChaincode("chaincodeNameOnChannelTwo", args, "channelTwo")

// 	if response.Status != shim.OK {
// 		errStr := fmt.Sprintf("Failed to invoke chaincode. Got error: %s", response.Message)
// 		fmt.Printf(errStr)
// 		return shim.Error(errStr)
// 	}

// 	// Handle the chaincode response
// 	fmt.Printf("Response from chaincode: %s", string(response.Payload))
